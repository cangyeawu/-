...
toggleSidebarBtn?.addEventListener('click',()=>{ sidebar.classList.toggle('open'); sidebar.classList.toggle('closed'); });
...
(function(){
  ...
  window.renderCalendar = async function renderCalendar(targetDate=new Date()){  
    ...
    const {weights,max} = await computeWeights(y,m);
    ...
    for(let d=1; d<=days; d++){  
      ...
      heat.style.background = `rgba(183,169,204, ${Math.round(ratio*80)/100})`;
...
(function(){
  ...
  function sortClips(arr){
    // match clipper.js ordering: first-clicked first (oldest first by createdAt)
    arr.sort((a,b)=>{
  ...
  async function openTheme(tid){
    ...
    btnRenameTheme.disabled=false; btnDeleteTheme.disabled=false;
...
(function(){
  ...
  function renderList(){
    if(!listEl) return;
    listEl.innerHTML = "";
    state.playlist.forEach((trk, i)=>{
      const item = document.createElement('div');
      item.className = 'item';
      item.tabIndex = 0;
      item.innerHTML = '<div class="title">'+ (i===state.index? '🎵 ' : '') + trk.name + '</div>';
  ...
  function loadFiles(fileList){
    const files = Array.from(fileList || [])
      .filter(f => /^audio\//.test(f.type) || /\.(mp3|wav|ogg|m4a|flac)$/i.test(f.name));
  ...
  function playPause(){
    if(audio.src==="" && state.playlist.length){
      playAt(state.index<0?0:state.index);
      return;
    }
  ...
  function next(){
    ...
    let i = state.index>=0? state.index+1 : 0;
    if(i>=state.playlist.length) i=0;
    playAt(i);
  }
  function prev(){
    if(!state.playlist.length) return;
    let i = state.index>=0? state.index-1 : state.playlist.length-1;
  ...
  function onTime(){
    ...
    if(isFinite(audio.duration) && audio.duration>0){
      ...
      seekBar.value = 0;
      timeLabel.textContent = fmt(0) + " / 00:00";
    }
  }
  ...
  // Re-render if user切换页面（保持状态）
  renderList();
  updatePlayIcon();

  // Cleanup object URLs when closing tab
...
// === mobile edge-swipe drawer (All Chats & Clips) ===
(function(){
  const isNarrow = ()=> window.innerWidth <= 900;
  const cfgs = [
    { sidebarId: 'chatSidebar', toggleId: 'toggleSidebar' },
    { sidebarId: 'themeSidebar', toggleId: 'toggleThemeSidebar' }
  ];

  function el(id){ return document.getElementById(id); }
  function isOpen(sb){ return sb?.classList.contains('open'); }
  function open(sb){
    // close others
    cfgs.forEach(c=>{
      const other = el(c.sidebarId);
      if(other && other!==sb){ other.classList.remove('open'); other.classList.add('closed'); }
    });
    sb?.classList.add('open'); sb?.classList.remove('closed');
  }
  function close(sb){ sb?.classList.remove('open'); sb?.classList.add('closed'); }
  function toggle(sb){ isOpen(sb) ? close(sb) : open(sb); }

  // Bind toggle buttons
  cfgs.forEach(c=>{
    const btn = el(c.toggleId), sb = el(c.sidebarId);
    if(btn && sb) btn.addEventListener('click', ()=> toggle(sb));
  });

  // default state
  function applyDefault(){
    cfgs.forEach(c=>{
      const sb = el(c.sidebarId);
      if(!sb) return;
      if(isNarrow()){ close(sb); } else { open(sb); }
    });
  }
  applyDefault();
  ...
  // gesture state
  let startX=0, startY=0, tracking=false, horiz=false, activeSidebar=null, activeCfg=null, edgeAttempt=false;

  document.addEventListener('touchstart', (e)=>{
    const t = e.touches && e.touches[0]; if(!t) return;
    startX = t.clientX; startY = t.clientY;
    tracking = false; horiz = false; activeSidebar = null; activeCfg = null; edgeAttempt = false;

    // if touch starts inside an open sidebar -> allow swipe to close
    for(const c of cfgs){
      const sb = el(c.sidebarId);
      if(!sb || !isOpen(sb)) continue;
      const r = sb.getBoundingClientRect();
      const inside = t.clientX >= r.left && t.clientX <= r.right && t.clientY >= r.top && t.clientY <= r.bottom;
      if(inside){ tracking = true; activeSidebar = sb; activeCfg = c; break; }
    }

    // otherwise, if from left edge (<=24px), set up to open preferred sidebar
    if(!tracking && startX <= 24){
      // prefer a sidebar that exists on the current page; All Chats first, otherwise Clips
      const pref = el('chatSidebar') ? el('chatSidebar') : (el('themeSidebar') ? el('themeSidebar') : null);
      if(pref){ tracking = true; activeSidebar = pref; activeCfg = cfgs.find(c=> el(c.sidebarId)===pref); edgeAttempt = true; }
    }
  }, {passive:true});

  document.addEventListener('touchmove', (e)=>{
    if(!tracking || !isNarrow()) return;
    const t = e.touches && e.touches[0]; if(!t) return;
    const dx = t.clientX - startX;
    const dy = t.clientY - startY;
    if(!horiz){
      if(Math.abs(dx) > Math.abs(dy)*1.2 && Math.abs(dx) > 6){ horiz = true; }
    }
  ...
  document.addEventListener('click', (e)=>{
    if(!isNarrow()) return;
    for(const c of cfgs){
      const sb = el(c.sidebarId), btn = el(c.toggleId);
      if(!sb || !isOpen(sb)) continue;
      const inside = sb.contains(e.target) || (btn && btn.contains(e.target));
      if(!inside) close(sb);
    }
  });
})();
...
// === Mobile drawer with edge-swipe gesture for sidebars ===
// Marker: Edge-swipe gesture for mobile drawers
(function(){
  ...
  function initMobileDrawer(asideId, toggleBtnId){
    const sidebar = document.getElementById(asideId);
    if(!sidebar) return;
    const toggleBtn = document.getElementById(toggleBtnId);
    const isNarrow = ()=> window.innerWidth <= 900;

    function open(){ sidebar.classList.add('open'); sidebar.classList.remove('closed'); }
    ...
    function applyDefault(){ if(isNarrow()){ close(); } else { open(); } }
    applyDefault();
    window.addEventListener('resize', applyDefault);

    // Edge swipe: from left edge open; when open, swipe left to close
    let startX=0, startY=0, tracking=false, fromEdge=false;
    document.addEventListener('touchstart', (e)=>{
      const t = e.touches && e.touches[0]; if(!t) return;
      startX = t.clientX; startY = t.clientY;
      fromEdge = startX < 24; // 24px 左缘热区
      const inside = sidebar.contains(document.elementFromPoint(startX, startY));
      tracking = isNarrow() && (fromEdge || (inside && sidebar.classList.contains('open')));
    }, {passive:true});
  ...
  // init for both sidebars if present
  initMobileDrawer('chatSidebar','toggleSidebar');
  initMobileDrawer('themeSidebar','toggleThemeSidebar');
})();


// === Mobile edge-swipe drawer (left) for chatSidebar & themeSidebar ===
(function(){
  if (window.__drawerGestureInitDone) return;
  window.__drawerGestureInitDone = true;
  ...
  function initDrawer(asideId, toggleBtnId){
    const sidebar = document.getElementById(asideId);
    if (!sidebar || sidebar.dataset.drawerInitialized === '1') return;
    sidebar.dataset.drawerInitialized = '1';
    const toggleBtn = document.getElementById(toggleBtnId);

    const isNarrow = ()=> window.innerWidth <= 900;
    const open = ()=>{ sidebar.classList.add('open'); sidebar.classList.remove('closed'); };
    const close = ()=>{ sidebar.classList.remove('open'); sidebar.classList.add('closed'); };
    ...
    // Edge swipe (from left edge to open; from inside to close)
    let startX=0, startY=0, tracking=false, fromEdge=false, fromInsideOpen=false;
    document.addEventListener('touchstart', (e)=>{
      if (!isNarrow()) return;
      const t = e.touches && e.touches[0]; if(!t) return;
      startX = t.clientX; startY = t.clientY;
      fromEdge = startX <= 20 && !sidebar.classList.contains('open');
      fromInsideOpen = sidebar.classList.contains('open') && sidebar.contains(document.elementFromPoint(startX, startY));
      tracking = fromEdge || fromInsideOpen;
    }, {passive:true});
    ...
    document.addEventListener('touchmove', (e)=>{
      ...
      const dx = t.clientX - startX;
      const dy = t.clientY - startY;
      // horizontal intent -> avoid page scroll jank
    ...
    document.addEventListener('touchend', (e)=>{
      ...
      const dx = t.clientX - startX;
      if(fromEdge && dx > 40){ open(); }
      if(fromInsideOpen && dx < -40){ close(); }
      tracking = false; fromEdge=false; fromInsideOpen=false;
    });
  ...
  initDrawer('chatSidebar','toggleSidebar');
  initDrawer('themeSidebar','toggleThemeSidebar');
})();
