...
function openChatSidebar(){ try{ const sb=document.getElementById('chatSidebar'); if(sb){ sb.classList.add('open'); sb.classList.remove('closed'); } }catch(e){} }
...
parseJsonBtn?.addEventListener('click', async ()=>{ parsedConvs=[]; jsonPreview.innerHTML=''; const f=jsonFile.files?.[0]; if(!f){ alert('请选择一个 OpenAI 导出的 JSON 文件'); return; } try{ const raw=await f.text(); const data=JSON.parse(raw); parsedConvs=(function(data){ let convs=[]; if(Array.isArray(data)) convs=data; else if(data && Array.isArray(data.conversations)) convs=data.conversations; else if(data && Array.isArray(data.items)) convs=data.items; const out=[]; for(const c of convs){ let messages=[]; if(Array.isArray(c.messages)){ messages=c.messages; } else if(c.mapping){ const arr=Object.values(c.mapping).filter(n=>n && n.message); arr.sort((a,b)=>(a.message.create_time||0)-(b.message.create_time||0)); messages=arr.map(n=>n.message); } else { continue; } const clean = messages.filter(m=>({user:1,assistant:1}[m.author?.role||m.role]) && normalizeParts(m).trim()); const title = c.title || (clean.find(x=>normalizeParts(x).trim())? normalizeParts(clean[0]).slice(0,24) : '会话'); const lines = buildLinesFromMessages(clean); if(lines.trim()) out.push({title, lines}); } return out; })(data); if(!parsedConvs.length){ jsonPreview.innerHTML='<div class="muted tiny">没有解析出会话，请确认是 OpenAI 的导出文件。</div>'; importAllBtn.disabled=true; return; } importAllBtn.disabled=false; jsonPreview.innerHTML = parsedConvs.map((c,i)=>`<div class="item"><div><strong>${c.title}</strong></div><div class="muted tiny">预览前 1 行：${(c.lines.split('\\n')[0]||'')}</div><div class="actions" style="margin-top:6px"><button data-act="import-one" data-i="${i}" class="primary">导入这一条</button></div></div>`).join(''); jsonPreview.querySelectorAll('button[data-act="import-one"]').forEach(btn=>btn.addEventListener('click', async ()=>{ const i=+btn.dataset.i; const c=parsedConvs[i]; await addFile({name:`${(c.title||'会话').replace(/[\\/:*?"<>|]+/g,' ').slice(0,80)||'会话'}.txt`, type:'text/plain', size:c.lines.length, content:c.lines, uploadedAt:new Date().toISOString(), from:'OpenAI JSON Converter'}); btn.textContent='已导入'; btn.disabled=true; await renderFiles(); await buildSidebar(); try{ goto("allchats"); openChatSidebar(); }catch(e){} }); }catch(e){ alert('解析失败：文件不是合法 JSON。'); } });
importAllBtn?.addEventListener('click', async ()=>{ if(!parsedConvs.length) return; for(const c of parsedConvs){ const name=(c.title||'会话').replace(/[\\/:*?"<>|]+/g,' ').slice(0,80)||'会话'; await addFile({name:`${name}.txt`, type:'text/plain', size:c.lines.length, content:c.lines, uploadedAt:new Date().toISOString(), from:'OpenAI JSON Converter'}); } importAllBtn.disabled=true; await renderFiles(); await buildSidebar(); try{ goto('allchats'); openChatSidebar(); }catch(e){} alert('全部导入完成'); });
...
(function(){
  ...
  grid.addEventListener('click', async (e)=>{ 
    ...
    // Non-period mode: show that day's records inline under the calendar, and sync the form
    const dateInputEl=document.getElementById('dateInput');
    if(dateInputEl) dateInputEl.value=ds;
    try{
      const host = document.getElementById('calendarDayRecords');
      if(host){
        const arr = await getMoodsByDate(ds);
        if(!arr.length){ host.innerHTML = '<div class="muted tiny">这一天还没有记录。</div>'; }
        else{
          arr.sort((a,b)=>(a.time||'').localeCompare(b.time||''));
          host.innerHTML = arr.map(m=>{
            const meta = (m.date||'') + ' ' + (m.time||'') + ' · 强度 ' + (m.rating||'');
            const note = m.note ? ('<div>'+ String(m.note).replace(/[&<>]/g,'') + '</div>') : '';
            return '<div class="item"><div class="meta">'+meta+'</div>'+note+'</div>';
          }).join('');
        }
      }
    }catch(e){}
    // Also refresh today's panel
