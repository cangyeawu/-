...
// Landing
const loveLines=["我把夜色拨开，只留一掌月光替我抱你。","想你时，风都学会了轻一点。","我偏爱你所有不合群的棱角，因为那都是你。"];
const comfortLines=["不急，先靠着我，呼吸跟着我来。","你不用解释，也不用表现得坚强，我懂。","把噪音交给我，你只要把自己交给晚上和睡眠。"];
...
toggleSidebarBtn?.addEventListener('click',()=>{
  ...
  const panel=document.getElementById('allchats');
  if(panel){ panel.classList.toggle('drawer-open', sidebar.classList.contains('open')); }
});
// overlay close on tap
document.getElementById('overlayAllChats')?.addEventListener('click',()=>{
  sidebar.classList.remove('open'); sidebar.classList.add('closed');
  const panel=document.getElementById('allchats'); if(panel) panel.classList.remove('drawer-open');
});
...
(function(){
  ...
  const tSidebarToggle = document.getElementById('tSidebarToggle');
  const overlayClips = document.getElementById('overlayClips');
  tSidebarToggle?.addEventListener('click',()=>{
    sidebar.classList.toggle('open'); sidebar.classList.toggle('closed');
    const panel=document.getElementById('clips');
    if(panel){ panel.classList.toggle('drawer-open', sidebar.classList.contains('open')); }
  });
  overlayClips?.addEventListener('click',()=>{
    sidebar.classList.remove('open'); sidebar.classList.add('closed');
    const panel=document.getElementById('clips'); if(panel) panel.classList.remove('drawer-open');
  });
