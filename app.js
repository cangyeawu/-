...
// === Theme meta-color sync (safe area/status bar color) ===
(function(){
  // Map theme keys to their background colors (must match CSS --bg vars)
  const THEME_BG = {
    "mori-oat":  "#ebe6dd",
    "mori-green":"#e6ebe4",
    "mori-blue": "#e6eaef",
  ...
  function setThemeMetaColor(theme){
    const color = THEME_BG[theme] || THEME_BG["mori-oat"];
  ...
  (function applySavedTheme(){
    ...
    const curr = saved || (document.documentElement.getAttribute('data-theme') || 'mori-oat');
    document.documentElement.setAttribute('data-theme', curr);
    setThemeMetaColor(curr);
  })();
  // hook swatches to also update meta color
  document.querySelectorAll('#styleTheme .swatch').forEach(btn=>{
...
importAllBtn?.addEventListener('click', async ()=>{ if(!parsedConvs.length) return; for(const c of parsedConvs){ const name=(c.title||'会话').replace(/[\\/:*?"<>|]+/g,' ').slice(0,80)||'会话'; await addFile({name:`${name}.txt`, type:'text/plain', size:c.lines.length, content:c.lines, uploadedAt:new Date().toISOString(), from:'OpenAI JSON Converter'}); } importAllBtn.disabled=true; await renderFiles(); await buildSidebar(); alert('全部导入完成'); try{ await buildSidebar(); }catch(e){} });
...
// Notifications (split anniv & countdown, independent toggles)
let annivNotiTimer = null;
let countdownNotiTimer = null;

function nextAnnivOccurrence(rec){
  // next date considering yearly repeat
  const base = new Date(rec.date);
  const now = new Date();
  if(rec.repeat === 'yearly'){
    const y = now.getFullYear();
    let next = new Date(y, base.getMonth(), base.getDate());
    const today = new Date(now.getFullYear(), now.getMonth(), now.getDate());
    if(next < today){ next = new Date(y+1, base.getMonth(), base.getDate()); }
    return next;
  }
  return base;
}

async function checkAnnivNotifications(){
  try{
    if(!('Notification' in window) || Notification.permission!=='granted') return;
    if(localStorage.getItem('noti:anniv:on')!=='1') return;
    const arr = (await getAllDates()).filter(x=> (x?.type||'anniv') === 'anniv');
    const now = new Date();
    for(const d of arr){
      const next = nextAnnivOccurrence(d);
      const due = new Date(next.getFullYear(), next.getMonth(), next.getDate(), 9, 0, 0); // 09:00
      if(Math.abs(due - now) < 60*1000){
        const key = `noti:anniv:${d.id}:${due.toDateString()}`;
        if(localStorage.getItem(key)) continue;
        new Notification(d.title || '纪念日提醒', {
          body: `${d.date}${d.repeat==='yearly'?'（每年）':''}`,
          silent: false
        });
...
async function checkCountdownNotifications(){
  try{
    if(!('Notification' in window) || Notification.permission!=='granted') return;
    if(localStorage.getItem('noti:countdown:on')!=='1') return;
    const arr = (await getAllDates()).filter(x=> (x?.type||'anniv') === 'countdown');
    const now = new Date();
    const today = new Date(now.getFullYear(), now.getMonth(), now.getDate());
    for(const d of arr){
      const base = new Date(d.date);
      const due = new Date(base.getFullYear(), base.getMonth(), base.getDate(), 9, 0, 0);
      // only fire on the target day at 09:00
      if(Math.abs(due - now) < 60*1000){
        const key = `noti:countdown:${d.id}:${due.toDateString()}`;
        if(localStorage.getItem(key)) continue;
        new Notification(d.title || '倒计时提醒', {
          body: `今天：${d.date}`,
          silent: false
        });
        localStorage.setItem(key,'1');
...
function startNotiWatchers(){
  if(annivNotiTimer) clearInterval(annivNotiTimer);
  if(countdownNotiTimer) clearInterval(countdownNotiTimer);
  // run once immediately to catch current minute
  checkAnnivNotifications();
  checkCountdownNotifications();
  // then every minute
  annivNotiTimer = setInterval(checkAnnivNotifications, 60000);
  countdownNotiTimer = setInterval(checkCountdownNotifications, 60000);
}

// Wire buttons
const __btnEnableAnniv = document.getElementById('enableNoti');
const __btnEnableCountdown = document.getElementById('enableNotiCountdown');

__btnEnableAnniv?.addEventListener('click', async ()=>{
  if(!('Notification' in window)) { alert('当前浏览器不支持通知'); return; }
  const p = await Notification.requestPermission();
  if(p==='granted'){
    localStorage.setItem('noti:anniv:on','1');
    alert('已为「纪念日」开启浏览器提醒');
    startNotiWatchers();
  }else{
    alert('未授权提醒');
  }
...
__btnEnableCountdown?.addEventListener('click', async ()=>{
  if(!('Notification' in window)) { alert('当前浏览器不支持通知'); return; }
  const p = await Notification.requestPermission();
  if(p==='granted'){
    localStorage.setItem('noti:countdown:on','1');
    alert('已为「倒计时」开启浏览器提醒');
    startNotiWatchers();
  }else{
    alert('未授权提醒');
  }
...
// auto-start if any toggles already on
if(localStorage.getItem('noti:anniv:on')==='1' || localStorage.getItem('noti:countdown:on')==='1'){
  startNotiWatchers();
}
...
(function(){
  ...
  saveBtn?.addEventListener('click', async ()=>{
    ...
    await renderToday(); try{ await renderCalendar(); }catch(e){}
// removed duplicate renderCalendar init call
  });
...
(function(){
  ...
  const toolsBox = document.getElementById('clipToolsVertical');
  const toolsToggle = document.getElementById('clipToolsToggle');
  ...
  try{
    if(typeof viewMode==='undefined' || viewMode!=='viewer') return;
    const curr = activeClips && activeClips[clipIndex] ? activeClips[clipIndex] : null;
    if(!curr) return;
    if(!confirm('删除这个剪藏/图片？')) return;
    await clips_del_item(curr.id);
    // refresh current theme list
    activeClips = await clipsOfTheme(activeThemeId);
    if(clipIndex >= activeClips.length) clipIndex = Math.max(0, activeClips.length - 1);
    if(!activeClips.length){
      renderThemeGrid();
    }else{
      await renderClip(clipIndex);
    }
    await buildSidebar();
  }catch(e){
    console.warn('deleteActiveClip failed', e);
  }
}
function ensureBackButton(mode){
    if(!clipMeta) return;
    let back = clipMeta.querySelector('button.back-grid');
    if(mode==='viewer'){
      if(!back){
        back = document.createElement('button');
        back.className = 'ghost tiny back-grid';
        back.textContent = '返回九宫格';
        back.addEventListener('click', ()=>{ renderThemeGrid(); });
        clipMeta.appendChild(back);
  ...
  function stashToolButtons(){
    try{
      const hiddenId = 'clipToolsHidden';
      let hidden = document.getElementById(hiddenId);
      if(!hidden){
        hidden = document.createElement('div');
        hidden.id = hiddenId;
        hidden.style.display = 'none';
        (stage||document.body).appendChild(hidden);
      }
      // move original actionable elements into hidden container
      ['themeImageInput','uploadImageBtn','reorderClipsBtn','renameThemeBtn','deleteThemeBtn'].forEach(id=>{
        const el = document.getElementById(id);
        if(el){ hidden.appendChild(el); }
      });
      // remove the vertical toolbox entirely to prevent flicker
      if(toolsBox && toolsBox.parentNode){ toolsBox.parentNode.removeChild(toolsBox); }
    }catch(e){ /* noop */ }
  }
  ...
  function ensureFooterActions(){
    if(!clipMeta) return;
    // ensure container
    let bar = clipMeta.querySelector('.clip-foot-actions');
    if(!bar){
      bar = document.createElement('div');
      bar.className = 'clip-foot-actions';
      clipMeta.prepend(bar);
    }
    // helper to ensure a button exists that proxies to original hidden toolbar button
    function ensureAction(id, label, kind){
      let btn = bar.querySelector('button[data-action="'+id+'"]');
      if(!btn){
        btn = document.createElement('button');
        btn.dataset.action = id;
        btn.className = (kind==='danger'?'danger':'ghost') + ' tiny';
    ...
    // create actions (upload -> reorder -> rename)
ensureAction('uploadImageBtn','上传图片','ghost');
ensureAction('reorderClipsBtn','重新排序','ghost');
ensureAction('renameThemeBtn','重命名','ghost');

// custom Delete: grid -> delete whole theme; viewer -> delete current clip only
let delBtn = bar.querySelector('button[data-action="__del"]');
if(!delBtn){
  delBtn = document.createElement('button');
  delBtn.dataset.action = '__del';
  delBtn.textContent = '删除';
  delBtn.className = 'danger';
  delBtn.addEventListener('click', ()=>{
    try{
      if(typeof viewMode !== 'undefined' && viewMode === 'viewer'){
        if(typeof deleteActiveClip === 'function'){ deleteActiveClip(); }
      }else{
        const orig = document.getElementById('deleteThemeBtn');
...
// keep the old vertical tools hidden but in DOM
if(toolsBox){ toolsBox.style.display='none'; }
  }

  const btnUploadImage = document.getElementById('uploadImageBtn');
  const inputThemeImage = document.getElementById('themeImageInput');
  const themeNoteWrap = document.getElementById('themeNoteWrap');
  const themeNoteInput = document.getElementById('themeNoteInput');
  const themeNoteSaved = document.getElementById('themeNoteSaved');


  if(!listHost) return; // not on this page
  if(toolsBox && toolsToggle){
    const KEY='clipsToolsCollapsed';
    try{ const saved=localStorage.getItem(KEY); if(saved==='1'){ toolsBox.classList.add('collapsed'); } }catch(e){}
    toolsToggle.addEventListener('click',()=>{ toolsBox.classList.toggle('collapsed'); try{ localStorage.setItem(KEY, toolsBox.classList.contains('collapsed')?'1':'0'); }catch(e){} });
  }
  if(btnUploadImage) btnUploadImage.disabled = true;
  if(themeNoteInput){ themeNoteInput.disabled = true; }


  // wire: upload image
  if(btnUploadImage && inputThemeImage){
    btnUploadImage.addEventListener('click', ()=>{
      if(!activeThemeId) return;
      inputThemeImage.value = '';
      inputThemeImage.click();
    });
    inputThemeImage.addEventListener('change', async (e)=>{
      const fs = Array.from(e.target.files||[]);
      if(fs.length){ await addImagesToActiveTheme(fs); }
    });
  }
  // wire: note input
  if(themeNoteInput){
    themeNoteInput.addEventListener('input', queueSaveNote);
    themeNoteInput.addEventListener('change', saveThemeNote);
  }
...
function readFileAsDataURL(file){
  ...
  async function maxOrderOfTheme(tid){
    const all = await clips_get_all();
    const arr = all.filter(x=>x && x.kind==='clip' && Number(x.themeId)===Number(tid));
    let maxo = 0; for(const c of arr){ const o = Number(c.order)||0; if(o>maxo) maxo = o; }
    return maxo;
  }

  async function addImagesToActiveTheme(files){
    if(!activeThemeId || !files || !files.length) return;
    const now = new Date().toISOString();
    let base = await maxOrderOfTheme(activeThemeId);
    for(const f of files){
      try{
        const dataUrl = await readFileAsDataURL(f);
        base += 1;
        await clips_add_item({
          kind: 'clip',
          themeId: Number(activeThemeId),
          createdAt: now,
          updatedAt: now,
          order: base,
          msgs: [],
        ...
        console.warn('Failed to import image', e);
      }
    }
    // refresh current theme list and render last added one
    activeClips = await clipsOfTheme(activeThemeId);
    clipIndex = Math.max(0, activeClips.length - 1);
    if(viewMode==='grid'){ renderThemeGrid(); } else { renderClip(clipIndex); }
  }

  // Note saving (debounced)
  let __noteTimer = null;
  async function saveThemeNote(){
    if(!activeThemeId) return;
    const all = await clips_get_all();
    const t = all.find(x=>x && x.kind==='theme' && x.id===activeThemeId);
    if(!t) return;
    t.note = String(themeNoteInput?.value||'').trim();
    t.updatedAt = new Date().toISOString();
    await clips_put_item(t);
    if(themeNoteSaved){
      themeNoteSaved.style.display = '';
      setTimeout(()=>{ themeNoteSaved.style.display='none'; }, 900);
    }
  }
  function queueSaveNote(){
    if(__noteTimer) clearTimeout(__noteTimer);
    __noteTimer = setTimeout(saveThemeNote, 500);
  }
function escapeHtml(s){ return (s||'').replace(/[<>&]/g, c=>({'<':'&lt;','>':'&gt;','&':'&amp;'}[c])); }
  ...
  async function openTheme(tid){
    ...
    // enable upload + load note
    if(btnUploadImage) btnUploadImage.disabled = false;
    if(themeNoteInput){
      themeNoteInput.disabled = false;
      themeNoteInput.value = t?.note || '';
    }
    ...
    clipIndex = 0;
    stashToolButtons(); renderThemeGrid();
  ...
  btnDeleteTheme?.addEventListener('click', async ()=>{
    ...
    activeThemeId = null; activeClips = []; clipIndex=0; if(btnUploadImage) btnUploadImage.disabled = true; if(themeNoteInput){ themeNoteInput.value=''; themeNoteInput.disabled=true; } titleEl.textContent='选择一个剪藏'; btnRenameTheme.disabled=true; btnDeleteTheme.disabled=true; renderClip(-1);
    await buildSidebar();
  });
  ...
  function renderThemeGrid(){
    ...
    if(!activeClips || !activeClips.length){
      clipContent.innerHTML = `<div class="theme-empty">（该剪藏暂无内容）</div>`;
      if(clipMeta) clipMeta.textContent = '';
      ensureFooterActions(); ensureBackButton('grid');
      return;
    }
    const total = activeClips.length;
    const max = 9;
    const firstNine = activeClips.slice(0, max);
    function cellThumb(c){
      if(c.img){
        return `<img class="tile-img" src="${c.img}" alt="${escapeHtml(c.imgName||'图片')}">`;
      }
      const txt = (c.text && String(c.text).trim()) ? String(c.text).trim()
                  : (Array.isArray(c.msgs) && c.msgs.length ? String(c.msgs[0]?.text||'').trim() : '(空剪藏)');
      const safe = escapeHtml(txt).slice(0, 140);
      return `<div class="tile-text"><div class="tile-text-inner">${safe}</div></div>`;
    }
    const cells = firstNine.map((c, i)=>{
      const overlay = (i===8 && total>max) ? `<div class="more-mask"><div class="count">${total}</div></div>` : '';
      return `<div class="grid-item" data-idx="${i}"><div class="thumb">${cellThumb(c)}</div>${overlay}</div>`;
    }).join('');
    clipContent.innerHTML = `<div class="theme-grid"><div class="grid">${cells}</div></div>`;
    if(clipMeta){
      clipMeta.innerHTML = `<span class="tiny muted">点击一个格子查看 · 共 ${total} 条</span>`;
    }
    ensureFooterActions(); ensureBackButton('grid');
    clipContent.querySelectorAll('.grid-item').forEach(el=>{
      el.addEventListener('click', ()=>{
        const i = Number(el.getAttribute('data-idx')||'0')|0;
        clipIndex = i;
...
async function renderClip(idx){
  viewMode='viewer'; if(prevBtn) prevBtn.style.display=''; if(nextBtn) nextBtn.style.display='';
clipContent.innerHTML = '';
  ...
  let inner = '';
  const map = await getParticipants?.() || new Map();
function sideOf(name){
  if(map && map.has && map.has(name)) return map.get(name).side || 'left';
  if(name==='苍夜') return 'left';
  if(name==='阿雾') return 'right';
  return 'left';
}
const bubbleOf = (who, text, ts)=>{
  const side = sideOf((who||'').trim());
  return `<div class="msg ${side}">
    <div class="bubble-col">
      <div class="bubble">${escapeHtml(text||'')}</div>
      ${ts ? `<div class="meta-bottom">${escapeHtml(ts||'')}</div>` : ``}
    </div>
  </div>`;
};
  ...
  if(c.img){
    inner = `<img class="clip-image" src="${c.img}" alt="${escapeHtml(c.imgName||'图片')}" />`;
  } else if(c.text && String(c.text).trim()){
  ...
  ensureFooterActions(); ensureBackButton('viewer');
ensureFooterActions(); ensureBackButton('viewer');
setClipContentDragSource();
}
  ...
  function clipPreviewOne(c){
    if(c && c.img){ return '<img class="thumb-img" alt="图片" src="'+c.img+'" />'; }
...
// === Font stack robustness for chat (override) ===
(function(){
  window.applyChatFont = function(code){
    let family = 'inherit';
    if(code === 'kaiti'){
      family = '"Kaiti SC","STKaiti","KaiTi","楷体","楷体_GB2312","Kaiti TC","BiauKai","AR PL KaitiM GB","Source Han Serif SC",serif';
    } else if(code === 'song'){
      family = '"Songti SC","STSong","SimSun","NSimSun","宋体","Songti TC","Source Han Serif SC","Noto Serif CJK SC",serif';
    } else if(code === 'rounded'){
      family = '"SF Pro Rounded","SF Pro Display","SF Pro Text","PingFang SC","MiSans","Nunito","Noto Sans SC","Helvetica Neue",sans-serif';
    } else {
      family = 'inherit';
    }
    document.documentElement.style.setProperty('--chat-font', family);
    try{ localStorage.setItem('chatFont', code); }catch(e){}
    try{ document.getElementById('chatFontSelSettings').value = code; }catch(e){}
  };
  const v = localStorage.getItem('chatFont') || 'default';
  window.applyChatFont(v);
})();

// ===== Homepage '提醒小猫' memo card =====
(function(){
  try{
    const wrap = document.getElementById('homeMemoWrap');
    if(!wrap) return;
    const body = document.getElementById('homeMemoBody');
    const savedEl = document.getElementById('homeMemoSaved');
    const KEY = 'homeMemoLinesV1';
    const MAX_LINES = 12;
    function debounce(fn, ms){ let t=null; return (...args)=>{ clearTimeout(t); t=setTimeout(()=>fn.apply(this,args),ms); }; }
    function load(){
      try{ const raw = localStorage.getItem(KEY); if(raw){ return JSON.parse(raw); } }catch(e){}
      return Array.from({length: 8}, ()=>({text:'', under:false}));
    }
    function save(lines){
      try{
        localStorage.setItem(KEY, JSON.stringify(lines));
        if(savedEl){ savedEl.style.display=''; setTimeout(()=>savedEl.style.display='none', 800); }
      }catch(e){}
    }
    function linesFromDOM(){
      const lines = [];
      body.querySelectorAll('.memo-line').forEach(li=>{
        const t = (li.querySelector('.text')?.textContent||'').trim();
        const under = li.classList.contains('under');
        lines.push({text:t, under});
      });
      return lines;
    }
    function addLine(text='', under=false, focus=false, afterEl=null){
      const li = document.createElement('div');
      li.className = 'memo-line' + (under?' under':'');
      li.innerHTML = '<div class="text" contenteditable="true" spellcheck="false"></div><button class="line-u" title="已完成/无需提醒（下划线）">U</button>';
      const tEl = li.querySelector('.text');
      const uBtn = li.querySelector('.line-u');
      tEl.textContent = text||'';
      const onSave = debounce(()=> save(linesFromDOM()), 300);
      uBtn.addEventListener('click', ()=>{ li.classList.toggle('under'); onSave(); });
      tEl.addEventListener('input', onSave);
      tEl.addEventListener('keydown', (e)=>{
        if(e.key==='Enter'){
          e.preventDefault();
          if(body.querySelectorAll('.memo-line').length < MAX_LINES){ addLine('', false, true, li); }
        }
      });
      if(afterEl && afterEl.nextSibling){ body.insertBefore(li, afterEl.nextSibling); }
      else if(afterEl){ body.appendChild(li); }
      else{ body.appendChild(li); }
      if(focus){ setTimeout(()=> tEl.focus(), 0); }
      return li;
    }
    function render(lines){
      body.innerHTML='';
      (lines||[]).forEach(l=> addLine(l.text, !!l.under, false));
      if(body.children.length===0) addLine('', false, false);
    }
    render(load());
  }catch(e){}
})();
...
// ===== Home hero memo toggle =====
(function(){
  try{
    const home = document.getElementById('home');
    const toggle = document.getElementById('homeMemoToggle');
    const wrap = document.getElementById('homeMemoWrap');
    if(home && toggle && wrap){
      // ensure collapsed initial state
      home.classList.remove('memo-open');
      wrap.style.maxHeight = '0px';
      function updateHeight(open){
        if(!open){
          wrap.style.maxHeight = '0px';
        ...
        const card = wrap.querySelector('.memo-card');
        let h = 0;
        if(card){
          // temporarily set to auto to measure
          const prev = wrap.style.maxHeight;
          wrap.style.maxHeight = 'none';
          h = card.offsetHeight + 8; // a little extra for margins
          wrap.style.maxHeight = prev;
        }else{
          h = 360;
        }
      ...
      toggle.addEventListener('click', ()=>{
        const open = !home.classList.contains('memo-open');
        home.classList.toggle('memo-open', open);
        updateHeight(open);
      });
      // On resize, if open, update the height (responsive)
      let ro;
      if('ResizeObserver' in window){
        ro = new ResizeObserver(()=>{
          if(home.classList.contains('memo-open')) updateHeight(true);
        });
        ro.observe(wrap);
      }else{
...
// ===== Clip vertical tools: drag + expand-up near bottom =====
(function(){
  try{
    const toolsBox = document.getElementById('clipToolsVertical');
    const toolsToggle = document.getElementById('clipToolsToggle');
    const stage = document.getElementById('themeStage') || document.body;
    if(!toolsBox) return;

    // restore last position
    (function restorePos(){
      try{
        const raw = localStorage.getItem('clipsToolsPosV1');
        if(raw){
          const pos = JSON.parse(raw);
          if(typeof pos.left === 'number' && typeof pos.top === 'number'){
            toolsBox.style.left = pos.left + 'px';
            toolsBox.style.top = pos.top + 'px';
            toolsBox.style.right = 'auto';
          }
        }
      }catch(e){}
    })();

    // drag (ignore buttons & expanded body)
    let dragging=false, startX=0, startY=0, boxX=0, boxY=0;
    toolsBox.addEventListener('pointerdown', (e)=>{
      const t = e.target;
      if (t.closest && (t.closest('.tools-body') || t.tagName === 'BUTTON')) return;
      dragging = true;
      try{ toolsBox.setPointerCapture(e.pointerId); }catch(err){}
      const rect = toolsBox.getBoundingClientRect();
      startX = e.clientX; startY = e.clientY;
      boxX = rect.left + (window.scrollX||0);
      boxY = rect.top + (window.scrollY||0);
      e.preventDefault();
    });
    window.addEventListener('pointermove', (e)=>{
      ...
      const srect = (stage.getBoundingClientRect ? stage.getBoundingClientRect() : {left:0, top:0});
      let x = boxX + dx - srect.left;
      let y = boxY + dy - srect.top;
      // constrain within viewport-ish
      const vw = (stage.clientWidth || window.innerWidth);
      const vh = (stage.clientHeight || window.innerHeight);
      const r = toolsBox.getBoundingClientRect();
      const minX = 8, minY = 8, maxX = vw - r.width - 8, maxY = vh - r.height - 8;
    ...
    window.addEventListener('pointerup', ()=>{
      ...
      dragging = false;
      try{
        const left = parseFloat(toolsBox.style.left)||0;
        const top = parseFloat(toolsBox.style.top)||0;
        localStorage.setItem('clipsToolsPosV1', JSON.stringify({left, top}));
      }catch(e){}
    });

    function measureBodyHeight(){
      const body = toolsBox.querySelector('.tools-body'); if(!body) return 0;
      const wasCollapsed = toolsBox.classList.contains('collapsed');
      let h = 0;
      if (wasCollapsed){
        toolsBox.classList.remove('collapsed');
        const prevVis = body.style.visibility;
        const prevDisp = body.style.display;
        body.style.visibility = 'hidden';
        body.style.display = 'flex';
        h = body.scrollHeight;
        body.style.visibility = prevVis;
        body.style.display = prevDisp;
        toolsBox.classList.add('collapsed');
      }else{
        h = body.scrollHeight;
      }
      return h;
    ...
    function applyDirection(){
      const body = toolsBox.querySelector('.tools-body'); if(!body) return;
      const rect = toolsBox.getBoundingClientRect();
      const bodyH = measureBodyHeight();
      const spaceBelow = window.innerHeight - rect.bottom;
      const spaceAbove = rect.top;
      if (bodyH + 12 > spaceBelow && spaceAbove >= spaceBelow){
        toolsBox.classList.add('expand-up');
        toolsBox.classList.remove('expand-down');
      }else{
        toolsBox.classList.add('expand-down');
        toolsBox.classList.remove('expand-up');
      }
    }

    // hook toggle: after original handler runs, decide direction
    if (toolsToggle){
      toolsToggle.addEventListener('click', ()=>{
        setTimeout(applyDirection, 0);
      });
    }
    window.addEventListener('resize', ()=>{
      if(!toolsBox.classList.contains('collapsed')) applyDirection();
    });
    // initial
    setTimeout(()=>{ if(!toolsBox.classList.contains('collapsed')) applyDirection(); }, 80);
  }catch(e){}
})();
...
(function(){
  try{
    const btn = document.getElementById('homeMemoClear');
    if(!btn) return;
    const KEY = 'homeMemoLinesV1';
    const savedEl = document.getElementById('homeMemoSaved');
    btn.addEventListener('click', ()=>{
      if(!confirm('确定清空所有提醒吗？')) return;
