...
// === vMobile Left-Drawer for sidebars (All Chats / Clips) ===
(function(){
  const isMobile = ()=> window.innerWidth <= 900;

  // Create a global mask once
  let mask = document.getElementById('sidebarMaskGlobal');
  if(!mask){
    mask = document.createElement('div');
    mask.id = 'sidebarMaskGlobal';
    mask.className = 'sidebar-mask';
    document.body.appendChild(mask);
  }

  // Utility: open/close a sidebar + mask
  function setSidebar(sidebar, open){
    if(!sidebar) return;
  ...
  function currentSidebar(){
    ...
    if(pid==='allchats') return document.getElementById('chatSidebar');
    if(pid==='clips') return document.getElementById('themeSidebar');
    return null;
  }

  // Hook existing toggle in All Chats
  ...
  // Inject a toggle button for Clips (剪藏) header if not present
  try{
    const clipsHeaderRow = document.querySelector('#clips .card .row.compact');
    if(clipsHeaderRow && !document.getElementById('toggleThemeSidebar')){
      const btn = document.createElement('button');
      btn.id = 'toggleThemeSidebar';
      btn.className = 'ghost';
      btn.textContent = '侧栏';
      btn.title = '展开/隐藏左侧剪藏侧栏';
      clipsHeaderRow.prepend(btn);
  ...
  // Clicking mask closes any open sidebar
  mask.addEventListener('click', ()=>{
    const sb = currentSidebar();
    setSidebar(sb, false);
  });

  // Edge-swipe to open; swipe from inside sidebar to close
  let touchStartX = 0, touchStartY = 0, swipeTarget = null, swiping = false, wantOpen = false, wantClose = false;
  ...
  function onTouchStart(e){
    ...
    const sb = currentSidebar();
    // Open gesture: start near left-edge <= 18px
    if(touchStartX <= 18){
      swipeTarget = sb;
      wantOpen = true; wantClose = false; swiping = true;
    }
    // Close gesture: start inside an open sidebar near its right edge (handle area 16px)
    else if(sb && sb.classList.contains('open')){
      const rect = sb.getBoundingClientRect();
      if(t.clientX >= rect.right - 24 && t.clientX <= rect.right + 4 && t.clientY >= rect.top && t.clientY <= rect.bottom){
        swipeTarget = sb; wantOpen = false; wantClose = true; swiping = true;
      }
    }
  }
  function onTouchMove(e){
    ...
    // small vertical move cancels
    if(Math.abs(t.clientY - touchStartY) > 26){ swiping = false; return; }
    if(wantOpen){
      // if horizontal swipe right beyond threshold, open
      if(dx > 30){ setSidebar(swipeTarget, true); swiping = false; }
    }else if(wantClose){
      // if swipe left beyond threshold, close
      if(dx < -30){ setSidebar(swipeTarget, false); swiping = false; }
    }
  }
  function onTouchEnd(){ swiping=false; swipeTarget=null; wantOpen=false; wantClose=false; }

  document.addEventListener('touchstart', onTouchStart, {passive:true});
  document.addEventListener('touchmove', onTouchMove, {passive:true});
  document.addEventListener('touchend', onTouchEnd, {passive:true});

  // Ensure correct mask state when switching tabs
  document.querySelectorAll('.tab').forEach(btn=>btn.addEventListener('click', ()=>{
    setTimeout(()=>{ mask.classList.remove('on'); }, 0);
  }));
})();
