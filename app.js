...
(function(){
  ...
  // ---- Make vertical tools draggable within themeStage ----
  (function(){
    const HANDLE = toolsToggle || toolsBox; // use toggle as drag handle
    const POS_KEY = 'clipsToolsPosV1';
    if(!toolsBox || !stage) return;
    toolsBox.style.right = ''; // we will control via left/top
    toolsBox.style.left = (toolsBox.style.left||'') || 'auto';
    toolsBox.style.top = (toolsBox.style.top||'') || '10px';
    toolsBox.style.touchAction = 'none';
    ...
    function applySaved(){
      try{
        ...
        if(!pos || typeof pos.xp!=='number' || typeof pos.yp!=='number') return;
        const rect = stage.getBoundingClientRect();
        const x = Math.max(0, Math.min(1, pos.xp)) * (rect.width - toolsBox.offsetWidth);
        const y = Math.max(0, Math.min(1, pos.yp)) * (rect.height - toolsBox.offsetHeight);
        toolsBox.style.left = Math.round(x) + 'px';
        toolsBox.style.top = Math.round(y) + 'px';
      }catch(e){}
    }
    function savePos(){
      try{
        const rect = stage.getBoundingClientRect();
        const bx = toolsBox.offsetLeft, by = toolsBox.offsetTop;
        const xp = rect.width>0 ? bx / (rect.width - toolsBox.offsetWidth) : 0;
        const yp = rect.height>0 ? by / (rect.height - toolsBox.offsetHeight) : 0;
        localStorage.setItem(POS_KEY, JSON.stringify({xp: Math.max(0,Math.min(1,xp||0)), yp: Math.max(0,Math.min(1,yp||0))}));
      }catch(e){}
    }
    ...
    function onDown(e){
      if(!HANDLE) return;
      dragging = true;
      toolsBox.classList.add('dragging');
      const r = stage.getBoundingClientRect();
      sx = e.clientX; sy = e.clientY;
      ox = toolsBox.offsetLeft; oy = toolsBox.offsetTop;
    ...
    function onMove(e){
      if(!dragging) return;
      const r = stage.getBoundingClientRect();
      const maxX = r.width - toolsBox.offsetWidth;
      const maxY = r.height - toolsBox.offsetHeight;
      const nx = clamp(ox + (e.clientX - sx), 0, Math.max(0, maxX));
      const ny = clamp(oy + (e.clientY - sy), 0, Math.max(0, maxY));
      toolsBox.style.left = Math.round(nx) + 'px';
      toolsBox.style.top = Math.round(ny) + 'px';
    }
    function onUp(e){
      if(!dragging) return;
      dragging = false;
      toolsBox.classList.remove('dragging');
...
(function(){
  try{
    ...
    render(load());
    // one-click clear
    const btnClear = document.getElementById('homeMemoClear');
    if(btnClear){
      btnClear.addEventListener('click', ()=>{
        if(!confirm('清空所有提醒？该操作不可恢复。')) return;
        const blank = Array.from({length: 8}, ()=>({text:'', under:false}));
        save(blank);
        render(blank);
      });
    }
