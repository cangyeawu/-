...
// === vMobile-iphone14pro: sync --topbar var & swipe drawer gestures ===
(function(){
  function updateTopbarVar(){
    try{
      const h = (document.querySelector('.topbar')?.offsetHeight || 64) + 0;
  ...
  function setupSwipeDrawer(sidebar, scope){
    if(!sidebar || !scope) return;
    let sx=0, sy=0, swiping=false, fromEdge=false;
    scope.addEventListener('touchstart', (e)=>{
      const t = e.touches && e.touches[0]; if(!t) return;
      sx = t.clientX; sy = t.clientY;
      const isOpen = sidebar.classList.contains('open');
      // open gesture: start near left edge in main content
      fromEdge = !isOpen && sx <= 16;
      // close gesture: start inside sidebar, swipe left
      if(isOpen && sidebar.contains(e.target)){ swiping = true; fromEdge = false; }
      else if(fromEdge){ swiping = true; }
      else { swiping = false; }
    }, {passive:true});
    ...
    scope.addEventListener('touchmove', (e)=>{
      ...
      if(Math.abs(dy) > Math.abs(dx)) { swiping=false; return; } // vertical scroll dominates
      if(fromEdge && dx > 40){
        sidebar.classList.add('open'); sidebar.classList.remove('closed');
        swiping = false;
      }else if(!fromEdge && dx < -40){
        sidebar.classList.remove('open'); sidebar.classList.add('closed');
        swiping = false;
      }
    }, {passive:true});

    scope.addEventListener('touchend', ()=>{ swiping=false; }, {passive:true});
  }

  // All Chats drawer
  try{
    const chatSidebarEl = document.getElementById('chatSidebar');
    setupSwipeDrawer(chatSidebarEl, document.getElementById('allchats') || document);
  }catch(e){}

  // Clips drawer
  try{
    const themeSidebarEl = document.getElementById('themeSidebar');
    setupSwipeDrawer(themeSidebarEl, document.getElementById('clips') || document);
  }catch(e){}
})();
