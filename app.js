...
// ===== Drawer (mobile) for sidebars + swipe gestures (ChatGPT-like) =====
(function(){
  ...
  updateTopbarH(); window.addEventListener('resize', updateTopbarH);

  // ensure a global backdrop
  ...
  function setupDrawer(aside, toggleBtn){
    if(!aside) return;
    function open(){ aside.classList.add('open'); aside.classList.remove('closed'); document.body.classList.add('drawer-open'); }
    function close(){ aside.classList.remove('open'); aside.classList.add('closed'); document.body.classList.remove('drawer-open'); }
    function toggle(){ (aside.classList.contains('open') ? close : open)(); }

    // default: phones start closed
    ...
    // edge-swipe: open from left edge; swipe-left to close when open
    let startX=0, startY=0, tracking=false, forClose=false;
    function onStart(e){
      const t = e.touches ? e.touches[0] : e;
      startX = t.clientX; startY = t.clientY;
      const fromEdge = startX < 20;
      const insideAside = aside.contains(t.target);
      if(!aside.classList.contains('open') && fromEdge){
        tracking = true; forClose = false;
      }else if(aside.classList.contains('open')){
        tracking = true; forClose = true;
      }else{
        tracking = false;
      }
    }
    function onMove(e){
      ...
      if(dy > 30) { tracking = false; return; } // don't hijack vertical scroll
      if(!aside.classList.contains('open') && dx > 40){ // open
        aside.classList.add('open'); aside.classList.remove('closed');
        document.body.classList.add('drawer-open');
      }
      if(forClose && dx < -40){ // close
        aside.classList.remove('open'); aside.classList.add('closed');
        document.body.classList.remove('drawer-open');
      }
    }
    function onEnd(){ tracking=false; forClose=false; }

    // attach on main content only
  ...
  // All Chats drawer
  const chatAside = document.getElementById('chatSidebar');
  const chatToggle = document.getElementById('toggleSidebar');
  const chatDrawer = setupDrawer(chatAside, chatToggle);

  // Themes (剪藏) drawer
  const themeAside = document.getElementById('themeSidebar');
  const themeToggle = document.getElementById('toggleThemeSidebar');
  const themeDrawer = setupDrawer(themeAside, themeToggle);

  // When switching tabs, close any opened drawer to avoid overlaying wrong panel
  tabs.forEach(btn=>btn.addEventListener('click', ()=>{
    document.body.classList.remove('drawer-open');
    [chatAside, themeAside].forEach(aside=>{ if(!aside) return; aside.classList.remove('open'); aside.classList.add('closed'); });
  }));
})();
