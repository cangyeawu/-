...
document.querySelectorAll('#styleTheme .swatch').forEach(btn=>btn.addEventListener('click',()=>{ const theme=btn.dataset.theme; document.documentElement.setAttribute('data-theme',theme); localStorage.setItem('theme',theme); try{ syncThemeColorMeta(); }catch(e){} }));
...
// === Sync <meta name="theme-color"> with current theme background ===
function syncThemeColorMeta(){
  try{
    const meta = document.querySelector('meta[name="theme-color"]');
    const bg = getComputedStyle(document.documentElement).getPropertyValue('--bg').trim() || '#ffffff';
    if(meta && meta.content !== bg){ meta.setAttribute('content', bg); }
    // also update #safeTop background via CSS var --bg (already follows theme)
  }catch(e){}
}
// call on load and when theme is changed
window.addEventListener('DOMContentLoaded', syncThemeColorMeta);
