...
(function(){
  ...
  async function doSaveToTheme(themeId, capturedText){
    let maxOrder = 0; try{ const all0 = await clips_get_all(); const arr0 = all0.filter(x=>x.kind==='clip' && Number(x.themeId)===Number(themeId)); maxOrder = Math.max(0, ...arr0.map(x=>Number.isFinite(+x.order)?+x.order:0)); }catch(_){ maxOrder = 0; }
    ...
    for(const it of ordered){
      if(it.type==='seg'){
        const payloadSel = { kind:'clip', themeId:Number(themeId), sourceFileId: currentFileId || null,
          createdAt:now, updatedAt:now, order:(maxOrder + (++seq)), msgs:[], text:it.text, preview: it.text.slice(0,120) };
        await clips_add(payloadSel); newClips += 1;
      } else if(it.type==='msg'){
        const m = it.data || {};
        const payloadMsg = { kind:'clip', themeId:Number(themeId), sourceFileId: currentFileId || null,
          createdAt:now, updatedAt:now, order:(maxOrder + (++seq)), msgs:[m], text:'', preview: (m.text||'').slice(0,120) };
